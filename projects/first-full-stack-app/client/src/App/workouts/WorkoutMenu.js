    ...
    render() {
        ...
        const allMoves = moves.map((move, i) =>
            <div draggable={true} className='wMMLItem' key={move + i}>
                ...
                {/* <div>{i+1}</div> */}
                <div>{move.name}</div>
                <button id={move._id + i} className='wMMLItemx' onClick={this.removeMove}>&times;</button>
            </div>
        )
        return (
            <div>
                <div className='popMenuItems'>
                    ...
                    {this.state.editing ?
                        <form className='editForm'>
                            ...
                            <button onClick={this.addMoves}>Add Moves</button>
                            {this.state.adding ? <div>
                                <p>Add Move From:</p>
                                <button onClick={this.addFromExisting}>Existing</button>
                                <button onClick={this.addNewMove}>New</button>
                            </div> : null}
                ...
                <ExistingMovesToAdd moves={this.props.data} /> 
                : null}
            </div>
        )
    }
